...
The platform MUST support OpenID Connect (OIDC) tokens in the `Authorization`. These tokens MUST be signed by a previously agreed OIDC server, to let the broker verify the token's validity. The service broker MAY need client credentials to communicate with the OIDC server with a service account.
...
| peeringPolicies* | array-of-strings | A list of peering policies that will be applied to the service plan and it's reflected on where the resources specified in the template are created. |
...
```
{
  "services": [{
    ...
    "plans": [{
      ...
      "peeringPolicies": ["Remote", "LocalAndRemote"],
      "free": false,
      "metadata": {
        "max_storage_tb": 5,
        "costs":[
...
When a broker receives a subscription request from the marketplace, it MUST
record the purchase inside a persistent system.
Usually this means that the purchase is recorded inside a database.
...
No parameters is present.

#### Authorization

Only a JWT token that present a role of `marketplace` is allowed to access this endpoint.
...
| service_id* | string | The ID of the service (from the catalog). MUST be globally unique. MUST be a non-empty string. |
| plan_id* | string | The ID of the plan (from the catalog) for which the service instance has been requested. MUST be unique within the service. MUST be a non-empty string. |
| user_id | string | The ID of the user who is subscribing to the service. MUST be a non-empty string. |
...
#### cURL
```
$ curl http://broker-url/service_subscription -d '{
  "service_id": "service-id-here",
  "plan_id": "plan-id-here",
  "plan_id": "user-id-here"
}' -X POST -H "X-Broker-API-Version: 2.13" -H "Content-Type: application/json" -H "Authorization: Bearer <JWT token>"
```
...
| Status Code | Description |
| --- | --- |
| 200 OK | MUST be returned if the service plan has been succesfully subscribed. |
| 400 Bad Request | MUST be returned if the request is malformed or missing mandatory data. |
| 409 Conflict | MUST be returned if a service plan has already been bought by that specific user |
...
| Response field | Type | Description |
| --- | --- | --- |
| subscription_id* | string | The id of the purchase. Must be unique inside the service broker |
...
When a broker receives a peering request from the marketplace or from a user, it MUST record the request inside a persistent system. Check if the Liqo peering towards the specified cluster_id has already been established and return its peering_id (associated to specific peering and namespace offloading). If the peering has not been established yet, the broker MUST create a new peering and return its peering_id, while starting the Liqo peering process.
...
Only a JWT token that present a role of `marketplace` is allowed to access this endpoint specifing any "user_id". If the user_id is not specified, the user_id of the user that is requesting the peering is taken from the Authorization header.
...
| Request field | Type | Description |
| --- | --- | --- |
| cluster_id* | string | The Liqo cluster id got from the liqoctl peer-command. |
| cluster_name* | string | The Liqo cluster name got from the liqoctl peer-command. |
| auth_url* | string | The Liqo authentication URL got from the liqoctl peer-command. |
| token* | string | The Liqo authentication token got from the liqoctl peer-command. |
| offloading_policy* | string | The policy to associate the peering to. MUST be "Local", "Remote" or "LocalAndRemote" as the Liqo standard defines. |
| user_id | string | The ID of the user who is requesting the peering for. It's required when the user in the Authorization header has a "marketplace" role |
| prefix_namespace | string | The eventual prefix to the namespace that will be created in the service broker cluster and peered cluster |
...
```
{
    ...
    "token": "token-here",
    "offloading_policy": "offloading-policy-here",
    "user_id": "cluster-id-here",
    "prefix_namespace": "prefix-namespace-here"
}
```

#### cURL
```
$ curl http://broker-url/peering -d '{
    "cluster_id": "cluster-id-here",
    "cluster_name": "cluster-name-here",
    "auth_url": "auth-url-here",
    "token": "token-here",
...
| Status Code | Description |
| --- | --- |
| 202 OK | MUST be returned if the peering has been started succesfully. |
| 400 Bad Request | MUST be returned if the request is malformed or missing mandatory data. |
| 409 Conflict | MUST be returned if a cluster id has already been peered by another user |
...
| Response field | Type | Description |
| --- | --- | --- |
| peering_id* | string | The id of the peering. Must be unique inside the service broker and it's associated to a specific peering with a specific cluster_id and a specific namespace |
...
#### Route
`GET /peering/:peering_id`

`:peering_id` MUST be a globally unique non-empty string.
It MUST be the ID a previously started peering operation.
...
Only a JWT token that present a role of `marketplace` is allowed to access this endpoint specifing any "peering_id". Otherwise, the user_id extracted from the token, MUST be the same as the user_id that requested the peering and associated with the it.
...
#### cURL
```
$ curl http://broker-url/peering/ -X GET -H "X-Broker-API-Version: 2.13" -H "Content-Type: application/json" -H "Authorization: Bearer <JWT token>"
```
...
| Status Code | Description |
| --- | --- |
| 202 Accepted | MUST be returned if the peering is in an ongoing state and no error occured. |
| 400 Bad Request | MUST be returned if the request is malformed or missing mandatory data or the peering does not exist yet. |
| 409 Conflict | MUST be returned if a cluster id has already been peered by another user |
| 500 Internal Server Error | MUST be returned if an error occured during the peering process |
...
| Response field | Type | Description |
| --- | --- | --- |
| namespace* | string | The namespace associated to the peering request |

\* Fields with an asterisk are REQUIRED.

```
{
  "namespace": "customer-asb42132345onsha23-2"
}
```

For an ongoing process response MUST return the following fields.

\* Fields with an asterisk are REQUIRED.

| Response field | Type | Description |
| --- | --- | --- |
| ready* | string | The status of the peering. Must "false". |
...
In case of an error response MUST return the following fields.

\* Fields with an asterisk are REQUIRED.

| Response field | Type | Description |
| --- | --- | --- |
| ready* | string | The status of the peering. Must be "false" |
